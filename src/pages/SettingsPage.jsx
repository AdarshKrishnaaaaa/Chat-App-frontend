        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-2 mb-6">
          {THEMES.map((t) => (
            <div
              key={t}
              onClick={() => setTheme(t)}
              className={`flex flex-col items-center gap-2 cursor-pointer transition-transform hover:scale-105 rounded-xl p-2 ${
                theme === t ? "ring-4 ring-primary" : ""
              }`}
            >
              <div
                className={`w-16 h-16 rounded-full border-2 ${
                  theme === t ? "border-primary" : "border-gray-300"
                }`}
                style={{
                  backgroundColor: `var(--${t}-bg)`,
                }}
              />
              <span className="text-sm capitalize text-center text-base-content">
                {t}
              </span>
            </div>
          ))}
